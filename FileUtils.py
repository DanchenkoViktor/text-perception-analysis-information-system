...
for author in authors:
    path_to_works_of_the_author = os.path.join(PATH_DATASET, author)
    _, _, filenames = next(walk(path_to_works_of_the_author))
    print(filenames)

    global_vacab = set()
    for filename in filenames:
        path = os.path.join(path_to_works_of_the_author, filename)
        # extract docx content
        doc_result = docx2python(path).text
        text = Precondition.clear_text_dataset(doc_result)
